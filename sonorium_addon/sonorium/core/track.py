"""
Audio Track Streaming Engine.

Core audio streaming module - platform agnostic.
Provides track playback streams with crossfading, sparse playback, and presence mixing.
...
try:
    ...
    av = None
    logger.warning("PyAV not installed - audio streaming will be disabled")
...
class TrackMetadata:
    ...
    def get_instance(self, theme=None):
        return TrackInstance(self, theme=theme)
    ...
    def duration_samples(self):
        ...
        if av is None:
            return SAMPLE_RATE * 60  # Fallback: assume 1 minute
...
class TrackInstance:
    ...
    def __init__(self, meta: TrackMetadata, theme=None):
        self.meta = meta
    ...
    def get_stream(self, exclusion_coordinator: ExclusionGroupCoordinator = None, random_start: bool = False):
        ...
        if self.crossfade_enabled:
            base_stream = CrossfadeTrackStream(self, random_start=random_start)
        else:
            base_stream = TrackStream(self, random_start=random_start)
...
class TrackStream:
    ...
    def __init__(self, instance: TrackInstance, random_start: bool = False):
    ...
    def _gen(self):
        ...
        if av is None:
            # No audio library - yield silence
        ...
        while True:
            try:
                ...
                if len(container.streams.audio) == 0:
                    logger.error(f'TrackStream: No audio stream in {self.instance.meta.path}')
                ...
                if first_loop and self.random_start and self.instance.meta.duration_seconds > 1.0:
                    ...
                    logger.debug(f'TrackStream: Random start at {seek_time:.1f}s for {self.instance.name}')
                    first_loop = False
                ...
                error_count += 1
                logger.error(f'TrackStream: Error playing {self.instance.name}: {e} (attempt {error_count}/{max_errors})')

                if error_count >= max_errors:
                    logger.error(f'TrackStream: Too many errors for {self.instance.name}, yielding silence indefinitely')
                    while True:
                        yield silence_chunk
...
class CrossfadeTrackStream:
    ...
    def _create_decoder(self, seek_random: bool = False):
        ...
        try:
            ...
            logger.error(f'CrossfadeTrackStream: Failed to open {self.instance.meta.path}: {e}')
        ...
        if len(container.streams.audio) == 0:
            logger.error(f'CrossfadeTrackStream: No audio stream in {self.instance.meta.path}')
            container.close()
            def empty_decoder():
                return
                yield
        ...
        if seek_random and self.instance.meta.duration_seconds > 1.0:
            try:
                ...
                seek_pts = int(seek_time / stream.time_base)
                container.seek(seek_pts, stream=stream)
                logger.debug(f'CrossfadeTrackStream: Random start at {seek_time:.1f}s for {self.instance.name}')
            except Exception as e:
                logger.warning(f'CrossfadeTrackStream: Failed to seek in {self.instance.name}: {e}')
        ...
        def decode():
            try:
                ...
                logger.error(f'CrossfadeTrackStream: Decode error in {self.instance.name}: {e}')
            finally:
                try:
                    container.close()
                except Exception:
...
class SparsePlaybackStream:
    ...
    def __init__(self, instance: TrackInstance, exclusion_coordinator: ExclusionGroupCoordinator = None):
        self.instance = instance
...
class PresenceMixingStream:
    ...
    def __init__(self, base_stream, instance: TrackInstance):
