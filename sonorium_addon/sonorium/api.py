...
class ApiSonorium(api.Base):
    ...
    async def _self_test_server(self):
        ...
        # Test 1: Check if configured port is bound
        try:
            ...
            sock.close()
            if result == 0:
                logger.info(f"  Self-test: Port {test_port} is OPEN and listening")
            else:
                logger.error(f"  Self-test: Port {test_port} connection failed (error {result})")
        except Exception as e:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get(f"http://127.0.0.1:{test_port}/health", timeout=5.0)
