"""
Theme Streaming Engine.

Core module for live theme audio streaming - platform agnostic.
Handles ThemeDefinition (what tracks to play) and ThemeStream (live audio generation).
...
from .recording import LOG_THRESHOLD, ExclusionGroupCoordinator, PlaybackMode
from .utils import IndexList, sanitize
...
class ThemeDefinition:
    ...
    def __init__(self, sonorium, name: str, theme_id: str = None):
        ...
        if hasattr(sonorium, 'theme_metas') and name in sonorium.theme_metas:
            theme_metas = sonorium.theme_metas[name]
        else:
            theme_metas = []
    ...
    def _get_metadata_path(self) -> Path:
        ...
        if hasattr(self.sonorium, 'path_audio'):
            return self.sonorium.path_audio / self.name / 'metadata.json'
        return Path(self.name) / 'metadata.json'
...
class ThemeStream:
    ...
    def chunk_silence(self):
        ...
        from .recording import TrackStream
        data = np.zeros((1, TrackStream.CHUNK_SIZE), np.int16)
        return data
    ...
    def __iter__(self):
        ...
        if av is None:
            logger.error('PyAV not available - cannot encode MP3 stream')
            return
