...
logger = logging.getLogger("sonorium.plugins.manager")


@runtime_checkable
class ConfigProtocol(Protocol):
    """Protocol for config objects that PluginManager accepts."""
...
class PluginManager:
    ...
    def __init__(
        ...
        self.audio_path = audio_path or Path(getattr(config, 'audio_path', 'themes'))
    ...
    async def initialize(self) -> None:
        ...
        enabled_list = getattr(self.config, 'enabled_plugins', [])
    ...
    def update_plugin_settings(self, plugin_id: str, settings: dict) -> bool:
        ...
        # Persist to config
        plugin_settings = getattr(self.config, 'plugin_settings', {})
        plugin_settings[plugin_id] = settings
        if hasattr(self.config, 'save'):
            self.config.save()
